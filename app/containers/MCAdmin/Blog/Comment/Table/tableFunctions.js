...
const tableFunctions = {
  ...
  ItemsConvert(header, body, items) {
    ...
    const groupedItems = [];
    const resultItems = [];
    items.forEach(() => { replyNumbers.push(0); });
    items.forEach((value) => {
      if (!value.ReplyToId) {
        groupedItems.push(value);
      } else {
        const index = groupedItems.findIndex((item) => item.Id === value.ReplyToId);
        groupedItems.splice(index + 1 + replyNumbers[index], 0, value);
        replyNumbers[index] += 1;
      }
    });

    groupedItems.forEach((value) => {
