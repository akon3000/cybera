...
import merchantwarriorIcon from '../../../../../assets/icon-payment-provider/merchant.jpg';

class Merchantwarrior extends Component {
  constructor(props) {
    super(props);
    this.state = {
      loading: false,
      providerName: 'Merchant Warrior',
      confirmRemove: false,
      MwHasRange: this.props.data !== null ? this.props.data.MwHasRange : false,
    };
  }
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.data !== this.props.data) {
      this.props.initialize({
        paymentGroup: nextProps.data.MwHasRange,
        MerchantUUID: nextProps.data.MwMerchantUUID,
        ApiKey: nextProps.data.MwApiKey,
        Passphrase: nextProps.data.MwApiPassphrase,
        PriceFrom: nextProps.data.MwRangeFrom === 0 ? nextProps.data.MwRangeFrom.toString() : nextProps.data.MwRangeFrom,
        PriceTo: nextProps.data.MwRangeTo === 0 ? nextProps.data.MwRangeTo.toString() : nextProps.data.MwRangeTo,
      });
    }
  }
  ...
  onRemove() {
    if (this.props.data !== null) {
      ...
      request.delete(`${apiUrl}/Websites/${localStorage.websiteID}/Payments/Merchantwarrior`, {}, (response) => {
  ...
  onSubmit(value) {
    this.setState({ loading: true });
    const data = {
      MwMerchantUUID: value.get('MerchantUUID'),
      MwApiKey: value.get('ApiKey'),
      MwApiPassphrase: value.get('Passphrase'),
      MwHasRange: value.get('paymentGroup'),
    };

    if (value.get('PriceFrom') !== null && value.get('paymentGroup')) data.MwRangeFrom = value.get('PriceFrom');
    if (value.get('PriceTo') !== null && value.get('paymentGroup')) data.MwRangeTo = value.get('PriceTo');

    if (this.props.data !== null) {
      request.put(`${apiUrl}/Websites/${localStorage.getItem('websiteID')}/Payments/Merchantwarrior`, data, (response) => {
        this.setState({ loading: false }, () => this.props.onSubmit(this.props.name, response, this.state.providerName));
      });
    } else {
      request.post(`${apiUrl}/Websites/${localStorage.getItem('websiteID')}/Payments/Merchantwarrior`, data, (response) => {
        this.setState({ loading: false }, () => this.props.onSubmit(this.props.name, response, this.state.providerName));
      });
    }
  }
  ...
  initData() {
    if (this.props.data !== null) {
      this.props.initialize({
        paymentGroup: this.props.data.MwHasRange,
        MerchantUUID: this.props.data.MwMerchantUUID,
        ApiKey: this.props.data.MwApiKey,
        Passphrase: this.props.data.MwApiPassphrase,
        PriceFrom: this.props.data.MwRangeFrom === 0 ? this.props.data.MwRangeFrom.toString() : this.props.data.MwRangeFrom,
        PriceTo: this.props.data.MwRangeTo === 0 ? this.props.data.MwRangeTo.toString() : this.props.data.MwRangeTo,
      });
    } else {
      this.props.initialize({
        paymentGroup: false,
  ...
  render() {
    ...
    return (
      <div className={styles.container} data-automation-id="option-merchantwarrior">

        { this.props.data !== null &&
          <Box className={styles.box}>
            <Row middle="xs">
              <Col xs={8} sm={8} md={9} lg={9} className={styles.settingIcon}>
                <img alt="merchantwarrior" src={merchantwarriorIcon} />
              </Col>
        ...
        <Dialog onClose={() => this.onClose()} title={this.state.providerName} open={this.props.open}>
          <Row className={styles.containerPopup}>
            <Col xs={12} sm={12} md={12} lg={12}>
              <Row>
                ...
                <Col lg={4} className={styles.providerLink}>
                  <a data-automation-id="link-to-merchantwarrior" target="_blank" href="https://www.merchantwarrior.com/">
            ...
            <Col xs={12} sm={12} md={12} lg={12}>
              <form onSubmit={this.props.handleSubmit((value) => this.onSubmit(value))} data-automation-id="form-merchantwarrior">
                <Row className={styles.row}>
                  <Col xs={7} sm={7} md={5} lg={5}>
                    <Field label="Merchant Id" name="MerchantUUID" type="text" component={input} data-automation-id="input-merchant-id" />
                  </Col>
                  <Col xs={7} sm={7} md={5} lg={5}>
                    <Field label="Api Key" name="ApiKey" type="text" component={input} data-automation-id="input-api-key" />
                  </Col>
                  <Col xs={7} sm={7} md={5} lg={5}>
                    <Field label="Api Passphrase" name="Passphrase" type="text" component={input} data-automation-id="input-api-passphrase" />
                  </Col>
                </Row>
                <Row className={styles.row}>
                  <Col xs={7} sm={7} md={5} lg={5}>
                    <Field
                      noneline
                      onChange={(event, value) => this.setState({ MwHasRange: value })}
...
const validate = (values) => {
  ...
  const requiredFields = {
    MerchantUUID(value) {
      ...
      const maxLength = Validate.isValidLength(value, 50, 'Merchant id');
      if (maxLength !== true) return maxLength;
      return true;
    },
    ApiKey(value) {
      if (!value) return message.format('require_enter', 'api key');
      const maxLength = Validate.isValidLength(value, 20, 'Api key');
      if (maxLength !== true) return maxLength;
      return true;
    },
    Passphrase(value) {
      if (!value) return message.format('require_enter', 'api passphrase');
      const maxLength = Validate.isValidLength(value, 50, 'Api passphrase');
      if (maxLength !== true) return maxLength;
      return true;
    },
  };
...
Merchantwarrior.defaultProps = {
  data: null,
  open: false,
};

export default reduxForm({
  form: 'Merchantwarrior',
  validate,
})(Merchantwarrior);
