...
class Paystream extends Component {
  constructor(props) {
    super(props);
    this.state = {
      loading: false,
      providerName: 'Paystream',
      confirmRemove: false,
      PayStreamHasRange: this.props.data !== null ? this.props.data.PayStreamHasRange : false,
    };
  }
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.data !== this.props.data) {
      this.props.initialize({
        paymentGroup: nextProps.data.PayStreamHasRange,
        UserName: nextProps.data.PayStreamUserName,
        Token: nextProps.data.PayStreamToken,
        SharedSecret: nextProps.data.PayStreamSharedSecret,
        PriceFrom: nextProps.data.PayStreamRangeFrom === 0 ? nextProps.data.PayStreamRangeFrom.toString() : nextProps.data.PayStreamRangeFrom,
        PriceTo: nextProps.data.PayStreamRangeTo === 0 ? nextProps.data.PayStreamRangeTo.toString() : nextProps.data.PayStreamRangeTo,
      });
    }
  }
  ...
  onClose() {
    this.initData();
    this.setState({
      PayStreamHasRange: this.props.data !== null ?
      this.props.data.PayStreamHasRange : false,
    });
    this.props.onClose();
  }
  ...
  onRemove() {
    if (this.props.data !== null) {
      ...
      request.delete(`${apiUrl}/Websites/${localStorage.websiteID}/Payments/Paystream`, {}, (response) => {
  ...
  onSubmit(value) {
    this.setState({ loading: true });
    const data = {
      PayStreamUserName: value.get('UserName'),
      PayStreamToken: value.get('Token'),
      PayStreamSharedSecret: value.get('SharedSecret'),
      PayStreamHasRange: value.get('paymentGroup'),
    };

    if (value.get('PriceFrom') !== null && value.get('paymentGroup')) data.PayStreamRangeFrom = value.get('PriceFrom');
    if (value.get('PriceTo') !== null && value.get('paymentGroup')) data.PayStreamRangeTo = value.get('PriceTo');

    if (this.props.data !== null) {
      request.put(`${apiUrl}/Websites/${localStorage.getItem('websiteID')}/Payments/Paystream`, data, (response) => {
        this.setState({ loading: false }, () => this.props.onSubmit(this.props.name, response, this.state.providerName));
      });
    } else {
      request.post(`${apiUrl}/Websites/${localStorage.getItem('websiteID')}/Payments/Paystream`, data, (response) => {
        this.setState({ loading: false }, () => this.props.onSubmit(this.props.name, response, this.state.providerName));
      });
    }
  }
  ...
  initData() {
    if (this.props.data !== null) {
      this.props.initialize({
        paymentGroup: this.props.data.PayStreamHasRange,
        UserName: this.props.data.PayStreamUserName,
        Token: this.props.data.PayStreamToken,
        SharedSecret: this.props.data.PayStreamSharedSecret,
        PriceFrom: this.props.data.PayStreamRangeFrom === 0 ? this.props.data.PayStreamRangeFrom.toString() : this.props.data.PayStreamRangeFrom,
        PriceTo: this.props.data.PayStreamRangeTo === 0 ? this.props.data.PayStreamRangeTo.toString() : this.props.data.PayStreamRangeTo,
      });
    } else {
      this.props.initialize({
        paymentGroup: false,
  ...
  render() {
    ...
    return (
      <div className={styles.container} data-automation-id="option-paystream">

        { this.props.data !== null &&
          <Box className={styles.box}>
            <Row middle="xs">
              <Col xs={8} sm={8} md={9} lg={9} className={styles.settingIcon}>
                <img alt="paystream" src={paystreamIcon} />
              </Col>
        ...
        <Dialog onClose={() => this.onClose()} title={this.state.providerName} open={this.props.open}>
          <Row className={styles.containerPopup}>
            <Col xs={12} sm={12} md={12} lg={12}>
              <Row>
                ...
                <Col lg={4} className={styles.providerLink}>
                  <a data-automation-id="link-to-paystream" target="_blank" href="https://www.paystream.com.au/">
            ...
            <Col xs={12} sm={12} md={12} lg={12}>
              <form onSubmit={this.props.handleSubmit((value) => this.onSubmit(value))} data-automation-id="form-paystream">
                <Row className={styles.row}>
                  ...
                  <Col xs={7} sm={7} md={5} lg={5}>
                    <Field label="Token" name="Token" type="text" component={input} data-automation-id="input-token" />
                  </Col>
                  <Col xs={7} sm={7} md={5} lg={5}>
                    <Field label="Shared Secret" name="SharedSecret" type="text" component={input} data-automation-id="input-shared-secret" />
                  </Col>
                </Row>
                <Row className={styles.row}>
                  <Col xs={7} sm={7} md={5} lg={5}>
                    <Field
                      noneline
                      onChange={(event, value) => this.setState({ PayStreamHasRange: value })}
...
const validate = (values) => {
  ...
  const requiredFields = {
    UserName(value) {
      ...
      const maxLength = Validate.isValidLength(value, 100, 'User name');
      if (maxLength !== true) return maxLength;
      return true;
    },
    Token(value) {
      if (!value) return message.format('require_enter', 'token');
      const maxLength = Validate.isValidLength(value, 100, 'Token');
      if (maxLength !== true) return maxLength;
      return true;
    },
    SharedSecret(value) {
      if (!value) return message.format('require_enter', 'shared Secret');
      const maxLength = Validate.isValidLength(value, 100, 'Shared Secret');
      if (maxLength !== true) return maxLength;
      return true;
    },
  };
