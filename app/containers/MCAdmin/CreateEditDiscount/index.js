...
export class CreateEditDiscount extends React.Component {
    ...
    if (this.state.codeInput === '') {
      ...
      if (this.state.pageFunction === 'Create') {
        const createBodyParams = {
          ...
          WebsiteId: parseInt(localStorage.getItem('websiteID'), 10),
        };
        request.post(
          `${apiUrl}/Discounts`,
          createBodyParams,
  ...
  render() {
    const breadCrumb = [
      <button data-automation-id="btn-merchant-discountdashboard-link" key="Shop" onClick={() => this.props.history.push('/MCAdmin/DiscountDashBoard')}>Shop</button>,
      <button data-automation-id="btn-merchant-discountdashboard-link" key="Discounts" onClick={() => this.props.history.push('/MCAdmin/DiscountDashBoard')}>Discount</button>,
      <button data-automation-id="btn-merchant-create-add-discount-link" key="Add_Edit_Discounts" onClick={() => window.location.reload()}>{(this.state.pageFunction === 'Create') ? 'Create New Discount' : 'Edit Discount'}</button>,
    ];
    ...
    if (this.state.DiscountPopupShow) {
      popup.push(
        <SuccessPopup
          ...
          data-automation-id="popup-merchant-discount-create-edit-success"
          onClose={() => {
            this.setState({ DiscountPopupShow: false });
            this.props.history.push('/MCAdmin/DiscountDashBoard');
          }}
        >
    ...
    if (this.state.ErrorPopupShow) {
      popup.push(
        <ErrorPopup
          data-automation-id="error-popup-merchant-discount-create-edit-fail"
    ...
    return (
      <Layout data-automation-id="page-merchant-create-add-discount" openedMenu="Shop" activeMenu="Discounts">
        <BreadCrumb data-automation-id="breadcrumb-merchant-create-add-discount" breadCrumb={breadCrumb} />
        <h2>{(this.state.pageFunction === 'Create') ? 'Create New Discount' : 'Edit Discount'}</h2>
        <Box>
          <form data-automation-id="form-merchant-create-add-discount" onSubmit={handleSubmit((values) => this.submit(values))}>
            <Row style={{ marginBottom: '20px' }}>
              <Col xs={12} md={6}>
                <Field data-automation-id="input-merchant-create-add-discount-name" label="Name" name="Name" type="text" component={Input} />
              </Col>
              <Col xs={12} md={6}>
                <div className={styles.textFieldContainer}>
                  <label htmlFor="Code">Code</label>
                  <TextField
                    data-automation-id="input-merchant-create-add-discount-code"
            ...
            <Row style={{ marginBottom: '10px' }}>
              <Col xs={12} md={6}>
                <Field data-automation-id="select-merchant-create-add-discount-usage" label="Usage" name="Usage" component={Select}>
              ...
              <Col xs={12} md={6}>
                <Field data-automation-id="select-merchant-create-add-discount-type" label="Type" name="Type" component={Select}>
            ...
            <Row style={{ marginBottom: '25px' }}>
              <Col xs={12} md={6}>
                <div className={styles.textFieldCodeContainer}>
                  ...
                  <TextField
                    data-automation-id="input-merchant-create-add-discount-minimum-amount"
              ...
              <Col xs={12} md={6} className={styles.AmountContainer}>
                <Field data-automation-id="input-merchant-create-add-discount-amount" label="Amount" name="Amount" type="text" component={Input} />
              </Col>
            </Row>
            <Row>
              <Col xs={12} md={6}>
                <Field data-automation-id="input-merchant-create-add-discount-startdate" label="from" name="StartDate" component={DatePickerInput} />
              </Col>
              <Col xs={12} md={6}>
                <Field data-automation-id="input-merchant-create-add-discount-enddate" label="to" name="EndDate" component={DatePickerInput} />
              </Col>
            </Row>
            <Row>
              <Col xs={12} md={6}>
                {(this.state.pageFunction === 'Edit') ?
                  <div className={styles.status}>
                    ...
                    <Toggle data-automation-id="input-merchant-create-add-discount-status" onClick={() => this.ToggleStatus()} defaultToggled={this.state.IsActive} />
                  </div>
                : null }
              </Col>
            </Row>
            <Row>
              <Col xs={12} sm={12} md={12}>
                <div className={styles.buttonFooter}>
                  <Button
                    data-automation-id="btn-merchant-create-add-discount-submit"
                    type="submit"
                    onClick={() => {
                      if (!this.state.codeInput) {
                        this.setState({ errorCodeInput: 'Please enter code' });
                  ...
                  >Save</Button>
                  <Button data-automation-id="btn-merchant-create-add-discount-cancel" type="button" btnStyle="negative" onClick={() => this.props.history.push('/MCAdmin/DiscountDashBoard')}>Cancel</Button>
                </div>
              </Col>
            </Row>
          </form>
