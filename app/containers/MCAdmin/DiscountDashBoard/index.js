...
export class DiscountDashBoard extends React.Component {
  ...
  render() {
    const breadCrumb = [
      <button data-automation-id="btn-merchant-discountdashboard-link" key="Shop" onClick={() => window.location.reload()}>Shop</button>,
      <button data-automation-id="btn-merchant-discountdashboard-link" key="Discounts" onClick={() => window.location.reload()}>Discount</button>,
    ];
    const header = [
    ...
    const body = [
      ...
      (value) => <Link
        to={{
          pathname: '/MCAdmin/CreateEditDiscount',
    ...
    return (
      <Layout data-automation-id="page-merchant-discountdashboard" openedMenu="Shop" activeMenu="Discounts">
        <BreadCrumb data-automation-id="breadcrumb-merchant-discountdashboard" breadCrumb={breadCrumb} />
        <h2>Discount</h2>
        <Box>
          <Row>
            <Col xs={12} sm={12} md={4} lg={4}>
              <div style={{ paddingBottom: '15px' }}>
                <Button data-automation-id="btn-merchant-add-new-discount" type="button" onClick={() => this.props.history.push('/MCAdmin/CreateEditDiscount')}><PlusIcon /> Create New discount</Button>
              </div>
            </Col>
          </Row>
          <form data-automation-id="form-merchant-discountdashboard-search" onSubmit={this.props.handleSubmit((values) => this.search(values.get('Name'), values.get('Code')))}>
            <Row end="xs">
              <Col xs={12} sm={4} lg={5}>
                <Field data-automation-id="input-merchant-discountdashboard-name" hintText="Name" name="Name" type="text" component={InputWithIcon} icon={<SearchIcon />} />
              </Col>
              <Col xs={12} sm={4} lg={5}>
                <Field data-automation-id="input-merchant-discountdashboard-code" hintText="Code" name="Code" type="text" component={InputWithIcon} icon={<SearchIcon />} />
              </Col>
              <Col xs={12} sm={4} lg={2}>
                <Button data-automation-id="btn-merchant-discountdashboard-search"><SearchIcon /> Search</Button>
              </Col>
            </Row>
          </form>
          <Table
            data-automation-id="table-merchant-discountdashboard"
            dataSource={`Discounts?$filter=WebsiteId eq ${localStorage.getItem('websiteID')}`}
            header={header}
            body={body}
            downloadBody={downloadBody}
            filter={this.state.filter}
            fileName="Merchant Discounts"
            onClearSearch={() => {
              this.setState({ filter: [] }, () => {
                this.props.reset();
              });
        ...
        { this.state.error &&
          <ErrorPopup
            data-automation-id="error-merchant-popup-discountdashboard"
