...
class SettingPopup extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (
        nextProps.popup === 'showSettingPopupHorizontalTab1' &&
  ...
  render() {
    ...
    return (
      <Dialog
        ...
        open={popup === 'showSettingPopupHorizontalTab1'}
        contentStyle={{ width: '750px' }}
        actions={[
          <PositiveButton
            onClick={() => onChangeSettingPopup(
              { ...this.props.setting, title: this.state.title, sizeMinTab: this.state.sizeMinTab },
      ...
      >
        <div className={styles.container}>
          ...
          <Row className={styles.rows}>
            ...
            <Col xs={6}>
              <label htmlFor="label">Tab min size</label>
              <Col xs={12}>
                <TextField
                  className={styles.fieldStyles}
              ...
              <Row middle="xs">
                {
                  this.state.tabs.map((x, idx) => (
                    <Col xs={10} key={`setting-tabs-data-${x.id}`}>
                      <span className={styles.label} htmlFor="icon-size">Tab {idx + 1}</span>
                      <TextField
                        className={styles.fieldStyles}
                        value={x.title}
                        type="text"
                ...
                <Col xs={12}>
                  <label className={styles.label} htmlFor="icon-size" style={{ fontSize: '22px' }}>
                    <AddTabIcon /> &nbsp;
                  </label>
                  { this.state.inputAdd }
                </Col>
